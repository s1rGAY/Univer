from Lab1.commands_realization import Commands_realization
from Lab1.linux_commands import Linux_commands
from Lab1.win_commands import Win_commands
from Lab1.macos_commands import macOS_commands
...
class Operation_system:
    ...
    def _run_command(self, command, path, argument):
        if self.commands.get_command(command) == 'clearing file':
            self.commands_sourse.clear_file(file_name=argument, path=path)
        elif self.commands.get_command(command) == 'remove file':
            self.commands_sourse.delete_file(file_name=argument, path=path)
        elif self.commands.get_command(command) == 'create file':
            self.commands_sourse.create_file(file_name=argument, path=path)
        elif self.commands.get_command(command) == 'overwrite file':
            self.commands_sourse.overwrite_file(file_name=argument, path=path)
        elif self.commands.get_command(command) == 'add data to end':
            self.commands_sourse.add_data_to_end(file_name=argument, path=path)
        elif self.commands.get_command(command) == 'view file data':
            self.commands_sourse.view_file_data(file_name=argument, path=path)
